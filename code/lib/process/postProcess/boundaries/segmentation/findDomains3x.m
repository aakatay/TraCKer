...
% find domains of a structure given recruitments
    ...
    for i = 1:numel(B)
        ...
        for j= 1:size(c,1)
            ...
            dy = c0y-c(j,1);
            if (abs(dx) + abs(dy)) == 3
                RbH(c(j,1)+dy/3,c(j,2)+dx/3)=1;
                RbH(c(j,1)+dy*2/3,c(j,2)+dx*2/3)=1;
            end
    ...
    for i = 1:ncb % each connecting branch
        ...
        for j = 1:nb % find connected branches
            ...
            if ~isempty(find(sbcOv==2)) % connected in HR
                sbc_rr = double(im2bw(binImage(sbc_,3),0));
                sbc_rrc = conv2(sbc_rr,ps,'same');
                sbc_rrc = im2bw(sbc_rrc,0);
                sbnc2rr = im2bw(binImage(sbnc2,3),0);
        ...
        sbcRR = sbcRR + im2bw(binImage(sbc_,3),0)*i; % real resolution
    end
