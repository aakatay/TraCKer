...
else % isSelectStructs=1
%
    figure(977); % subplot images
    gsb=(findall(gcf,'type','axes')); % sub plots
    ...
    for isb = 1:nsb
        axes(gsb(isb))
        hold on;
        scatter(xd0-xl2(1)+1,yd0-yl2(1)+1,'r','.')
        scatter(xc0-xl2(1)+1,yc0-yl2(1)+1,'c','.')
        hold off;
    end
end
...
if ~tDiffSingleFrame, p3=zeros(size(p2)); p4 =p3; end;
...
if isReAdjustProfLines & skipGoodOnes & numel(p2)>8 % skip if done already
    isContinue = 1; return;
end

nl = numel(p0); % length
mx = ceil(nl/2); % mid index
pdiff = p0(mx)-p2(mx);
if pdiff<pdiffThresh
    warning('not huge pinchoff, skipping')
    isContinue = 1; return;
end
