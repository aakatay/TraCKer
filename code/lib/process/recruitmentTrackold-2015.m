...
last = last_(end-2:end-1);
nmCrop = fname(last(1)+1:last(2)-1);
ixX = find(nmCrop=='X');
ixY = find(nmCrop=='Y');
ixx = find(nmCrop=='x');
...
if isTrace>1
    ...
    %% discard (1) short traces and (2) spread traces
    trInf = trInf(trInf(:,8)<=cfg.trace.minXYspread,:); % discard spread traces
    trInfDiscard = trInf(trInf(:,8)>cfg.trace.minXYspread,:); % discard spread traces
    %trace length
    figure;
