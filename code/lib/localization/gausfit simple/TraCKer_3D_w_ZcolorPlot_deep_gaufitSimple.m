...
clear; close all;
    ...
    cfg.trace.isCombTraces = isCombTraces;
    cfg.report = struct;
    ...
    if ~isempty(msPos2)
        ...
        error('acquisition time info is missing')
    end

    % upd binFrame (binning of the frames)
    if binFrameTime
        binFrame = ceil(binFrameTime/acqTime/1000); % [frames]
    else
    ...
    coeffMat_ = dir('coeff*.mat');
    coeffMat = [];
    Coeff = 0;
    
    coeffFound = 0;
    if ~isempty(coeffMat_)
        if numel(coeffMat_) > 1
            for i = 1:numel(coeffMat_)
                if strcmp(fname(end-6:end-4),coeffMat_(i).name(end-6:end-4))
            ...
            if coeffFound == 1
                coeffMat = coeffMat_(iCoeff).name;
            end
        elseif length(coeffMat_.name) == 13 && strcmp(fname(end-6:end-4),coeffMat_.name(end-6:end-4))
            coeffMat = coeffMat_.name;
            coeffFound = 1;
        else
    ...
    if ~exist(spotWinFileNm) 
        ...
        if ~coeffFound
            ...
            while q == 0 
                ...
                [y,x,v]=find(BWSHOW==1);
                if numel(BWSHOW(:)) == sum(BWSHOW(:))
                    x=[]; y=[];
                end;
                save('filt1','BWSHOW','BINAR','PREdataFiltTiledDiv','PREdataFiltTiled1','DINAR1','Coeff')
%close all;
%return
                figure(hFigDisp);
                ...
                if nTile>round(Frames/nFrTile)-1
                    nTile = round(Frames/nFrTile)-1;
                end
        ...
        for k=1:Frames-frstFrm+1
            ...
            for iAv = 1 : binFrame
                frmRead = (k+frstFrm-2)*binFrame+iAv;
                temp(:,:,iAv) = double(imread(fname,frmRead)); 
                temp2(:,:,iAv) = double(imread(fnameBck,frmRead)); 
            end
            IMG = mean(temp(yy1:yy2,xx1:xx2,:),3);
            
            IMGsum = IMG + IMGsum;
            if rem(k,numFrmInOneMin) == 0
                imwrite(IMGsum,'blockAvOneMin.tif','WriteMode','append')
                IMGsum = 0;
            end
            temp2 = uint16(mean(temp2,3).*BWbckgrnd);   
...
%             din2 = uint16(bin).*IMG; 
            din2 = 0;
            if isempty(find( din1 + din2 > 0, 1)), continue; end;
...
%return
            ...
            %end
                for i = 1:q-1 % merging 1 pixel separated spots (see regressSpotDetection)
                    x1 = double(xBW(i,k));
                    x2 = double(PxBW);
                    y1 = double(yBW(i,k));
                    y2 = double(PyBW);
                    isMerge = 0;
                    if (x1-x2)^2+(y1-y2)^2 <= 5 % overlapping spots
                        isPreMerge = find(mergeQ==i);
                        if isempty(isPreMerge)
                            I1=IMG(yBW(i,k),xBW(i,k));
                        else
                            I1 = mergeInt(isPreMerge);
                        end
                        I2=IMG(PyBW,PxBW);
                        xBW_ = round((double(xBW(i,k))*I1+double(PxBW)*I2)/(I1+I2));
                        yBW_ = round((double(yBW(i,k))*I1+double(PyBW)*I2)/(I1+I2));

                        xBW(i,k) = xBW_;
                        yBW(i,k) = yBW_; 
                        isDoubleHeadedSpot(i,k) = true;
                        q = q-1;
                        mergeQ=i;
                        isMerge = 1;
                        mergeInt = (I1+I2);
            ...
            % ignore the spots on the edge and find the big window
            nSpotsInThisFrame = sum(xBW(:,k)>0); 
            spIx = 1;
            while spIx <= nSpotsInThisFrame
                if ((xBW(spIx,k)-(BigWindowSize+1)/2)<1 || (yBW(spIx,k)-(BigWindowSize+1)/2)<1 || (xBW(spIx,k)+(BigWindowSize+1)/2)>En1 || (yBW(spIx,k)+(BigWindowSize+1)/2)>Boy1)
                    if spIx > nSpotsInThisFrame
                        xBW(end,k) = 0;
                        yBW(end,k) = 0;
                    else 
                        xBW(spIx:end-1,k) = xBW(spIx+1:end,k);
                        yBW(spIx:end-1,k) = yBW(spIx+1:end,k);          
                        xBW(end,k) = 0;
                        yBW(end,k) = 0;          
                    end
                    nSpotsInThisFrame = nSpotsInThisFrame-1;
                    continue;
                end            
                %DEFINE Big Window
                yy = yBW(spIx,k)-(BigWindowSize+1)/2;
                y1 = yy + 1; y2 = yy + BigWindowSize;
                xx = xBW(spIx,k)-(BigWindowSize+1)/2;
                x1 = xx + 1; x2 = xx + BigWindowSize;
                BigWindow=IMG(y1:y2,x1:x2);   
                
                spotWin(:,:,sp) = BigWindow*bckFirst/bck(k);  
                spIx = spIx + 1; % counting spots in this frame 
                sp = sp + 1;
            end
            ss = sum(sum(xBW>0));
                if size(spotWin,3) ~= sum(sum(xBW>0))
                    pause;
                end
            zBW=zeros(size(xBW)); 
        ...
        cfg.report.spots_numDetected = numel(xBW(xBW>0));
        cfg.report.spots_ratioDoubleHeaded = sum(isDoubleHeadedSpot(:))/sum(sum(xBW>0));
        save(spotWinFileNm,'spotWin','xBW','yBW','zBW','cfg','isDoubleHeadedSpot'); 
    ...
    %% DOUBLE GAUS FIT
    if isGausFitLocalize && ~exist(xyzDataGausFileNm,'file')
        % load data
        fnSpot = rdir('spotWin*');
...
%             pxYc = round(cc1(5,sp));
            ...
            if 1 || pxXc==1 || pxYc==1 || pxXc==m || pxYc==m  % 1Gfit peak is on the edge
%                 is1GinCenter(sp) = 0;
...
%                 yPre2(sp) = yPre2disp(sp)-2;
            ...
            %% for display only
            viewSpot2dispLoop;
            cfg.report.fit_numIsTryFit0 = numel(isTryFit(isTryFit == 0));
            cfg.report.fit_numIsTryFit1 = numel(isTryFit(isTryFit == 1));
        ...
        save(xyzDataGausFileNm,'Xgaus','Ygaus','Zgaus','INTnorm','cfg');
        viewSpot3dispSave;
        close(h);
    end
    
    
    if isGausFitLocalize && ~exist(xyzDataGausFiltFileNm,'file')
        assignFileNames
        ...
        if ~exist('spotSelVal.mat') % by selecting the threshold values
            ...
            cfg.fit.dblG.errThreshold = errThreshold;
            cfg.fit.dblG.intThreshold = intThreshold;
            load spotInfo
        ...
        cfg.report.gausFitEliminationRate = 1-(numel(ixSel)/numel(intFit));
        cfg.report.numSpotFit = numel(ixSel);
        cfg.report.numSpot2Fit = numel(intFit);
        % filter
        ...
        X = Xgaus; Y = Ygaus; Z = Zgaus; clear Xgaus Ygaus Zgaus;
        
        %%
report = 0;        
if report        
        load(spotWinFileNm, 'xBW');
        load(posDataFileNm, 'bck')
        smthNum = 1000;
        spotPerFrm = smooth(sum(xBW>0,1),smthNum);
        fitPerFrm = smooth(sum(X>0,1),smthNum);
        
        hReport = figure;
        subplot(2,3,1);
        k=size(xBW,2);
        spotFail2Fit = smooth(sum(xBW>0,1),smthNum) - smooth(sum(X>0,1),smthNum);
        [hAx,hLine1,hLine2]=plotyy(1:k,smooth(sum(xBW>0,1),smthNum),1:k,smooth(bck,smthNum)); %axis square; 
        xlabel('frames');
        ylabel(hAx(1),'spots'); ylabel(hAx(2),'background intensity')
        title('(1)detected spots (2) bakcground intensity vs time')
        hold on;
        plot(smooth(sum(X>0,1),smthNum),'r');
        plot(spotFail2Fit*5,'c');
        legend('detected spots','successful fits','(spots-fits)*5','background intensity')
        
        hold off;
        
        subplot(2,3,2);
        plot(fitPerFrm./spotPerFrm*100);
        xlabel('frames');ylabel('spots');title('the percentage of successful fits to spots')
        gfrm = getframe(gcf);
        cfg.report.spots_imgSpy=gfrm.cdata;
        
        subplot(2,3,3);
        meanSpotInt=sum(INT,1)./sum(INT>0,1);
        plot(smooth(meanSpotInt,smthNum));
        savefig(hReport,'report.fig')
        %%
end        
        
        INT = INTnorm; clear INTnorm;
        save(xyzDataGausFiltFileNm,'X','Y','Z','INT','cfg');
    ...
    if ~exist(traceDataFileNm0,'file')
        ...
        elseif exist(xyzDataGausFileNm,'file')
        ...
        if ~exist('Z','variable'), Z = zeros(size(X)); end;
    ...
    if 0 && ~exist(traceDataFileNm,'file') && isCombTraces
        %% COMBINE TRACES
        ...
        %     TraceDif1=0;TraceDif2=0;TraceDif3=0;
            ...
            for j=i+1:Boy2
                FirstElement=find(TraceX(j,:)>0, 1 );
    ...
    % PLOT TRACES
        ...
        %frames = 1000;
        TraceX = TraceX(:,1:frames);
        TraceY = TraceY(:,1:frames);
        TraceZ = TraceZ(:,1:frames);
        ...
        close(hWBfillgaps)
        toc
        save(traceJmplessDataFileNm,'TraceX','TraceY','TraceZ')
    end
    ...
    padSize = 0;
    if exist(xyzDataGausFileNm,'file'), load(xyzDataGausFileNm); else load(xyzDataFileNm); end;
    zeroIx = find(X == 0);
...
%% PLOT    
    ...
    for ixFrm = frm1:En
        ...
        hscat = scatter(X(Y(:,ixFrm)>0,ixFrm),imSz(2)-Y(Y(:,ixFrm)>0,ixFrm)+1,50,'r','d','LineWidth',0.5);
