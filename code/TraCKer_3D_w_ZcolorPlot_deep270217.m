...
%minXYspread = 1.6; % [px] std of the xy location along the trace
    ...
    inFocus_w_1X = 0; % 1.5x mag not used 
    isDiffData = 0;
...
%         gausKernelSg = 1;
        ...
        if ~skipAdjCoeff && ~isOptimFit
            ...
            while q == 0 
                ...
                updCoeff(hCoeff, 0,hFigDisp,hTextCoeff,hscat,hscat2,hscat3,CoeffFit,sizeImg,nRow,IMGtiled,IMGfiltTiled,filtMax,isDebugFilt,frstFrm,nTile,nFrTile,isRangeLow)
                pause(0.1)
            end % while loop
