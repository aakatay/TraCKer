...
for i = 1:nfr % each frame
    ...
    smMapCv = conv2(smMap,ones(8),'same');
    smMapCv(smMapCv~=1) = -10000;
    smMapCv2 = conv2(smMapCv,ones(8),'same');
    smMapCv3 = zeros(szXY);
    ...
    if i >= stdWin % calculate SNR
        ...
        for j = 1:numel(aIXix) % for each SM 
            ...
            if size(asm,3) ~= i, smLast = smLast*nan; end;
            ...
            xyix = trInf(ix,3); % index to X Y arrays
            Xs(m) = X(xyix);
            Ys(m) = Y(xyix);
            Frm(m) = i;
            TRinf(m,:) = trInf(ix,:);     
