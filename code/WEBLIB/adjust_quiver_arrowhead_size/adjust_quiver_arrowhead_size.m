function adjust_quiver_arrowhead_size(quivergroup_handle, delta)
...
if ~exist('delta', 'var') || ...
   isempty(delta) || ...
   ~isnumeric(delta)
    errordlg('Input "delta" is missing, empty or non-numeric.', ...
             mfilename);
    return
end

if length(delta) > 1
    errordlg('Expected "delta" to be a scalar.', mfilename);
    return
end

if delta <= 0
    errordlg('"delta" should be > 0.', mfilename);
    return
end
...
for index = first_nan_index : 4 : last_nan_index
    ...
    delta0 = sqrt(delta_x^2+ delta_y^2);
    scaling_factor = delta/delta0;
